...
get_header();
$code = get_field('agent_code');
// Then display it visibly on the page
echo '<div style="background: #eee; padding: 10px; margin: 10px;">Agent Code: ';
var_dump($code);
echo '</div>';
$count = json_decode(file_get_contents("https://realestate.marketingwebsites.ca/api.php/properties?agents=$code"), 1)['results_count'];
?>
...
<?php if ($count && $code): ?>
    <section class="explore member--listings padd-lg">
        <div class="container">
            ...
            <div class="row mt-4">
                <?= do_shortcode('[properties template="grid" status="EV" order="price" dir="desc" pagination="1" agents="' . esc_attr($code) . '"]'); 
                ?>
            </div>
        </div>
    </section>
