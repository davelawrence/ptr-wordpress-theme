...
$button_text = get_sub_field('button_text');
$button_url = get_sub_field('button_url');
$municipality = get_sub_field('municipality_code');
?>
<section class="explore propeties-section newpage">
	<div class="container">
		...
		<div class="row row-cols-md-3 row-cols-1 mt-5">
			<?= do_shortcode('[properties template="grid" limit="6" order="rand" status="EV" municipalities="' . $municipality . '"]') ?>
		</div>
	</div>
</section>
<?php endwhile;?>
...
<section class="latest-real" data-aos="fade-up" data-aos-delay="200">
  ...
  <?php 
    // these should both be URL strings now
    $video_url = get_sub_field('market_stats_video'); 
  ...
  <?php if ( $video_url ): ?>
    <div class="stats-media">
      <?php 
        // WP will fetch & cache the proper <iframe> for you
...
<?php endwhile;?>
<?php endif;?>
<!---<?php if( have_rows('agent_details') ): ?>
<?php while( have_rows('agent_details') ): the_row();
$content = get_sub_field('content');
$image = get_sub_field('image');
$name = get_sub_field('name');
?>
...
?>
<div class="icons  <?php if($pcounter==1){?> first <?php } ?>">
<img src="<?php echo $icon;?>" alt=""> <a href="<?php echo $url;?>"><?php echo $value;?></a>
</div>
<?php $pcounter++;?>
<?php endwhile;?>
<?php endif;?>
...
<section class="about-john" data-aos="fade-up" data-aos-delay="200">
	<div class="container">
		<div class="row align-items-center">
			...
			<div class="col-lg-8 col-md-7 col-12">
				<div class="section-title">
					<?php $author_quote = get_field('author_quote');?>
					<?php echo $author_quote;?>
					<h2><?php echo esc_html($related_post->post_title); ?></h2>
					<?php if($designation){?>
					<div class="icons  first ">
						<img src="<?php echo get_site_url(); ?>/wp-content/uploads/2025/02/icon-location-dark.png.webp" alt=""> <a href="#"><?php echo $designation;?></a>
					</div>
					<?php } ?>
					<?php if($phone){?>
					<div class="icons ">
						<img src="<?php echo get_site_url(); ?>/wp-content/uploads/2025/02/icon-call.svg" alt=""> <a href="tel:<?php echo $phone;?>"><?php echo $phone;?></a>
					</div>
					<?php } ?>
					<?php if($email){?>
					<div class="icons ">
						<img src="<?php echo get_site_url(); ?>/wp-content/uploads/2025/02/icon-email.svg" alt=""> <a href="mailto:<?php echo $email;?>"><?php echo $email;?></a>
					</div>
					<?php } ?>
				</div>
			</div>
		...
		<div class="row justify-content-center">
			<div class="col-lg-8 col-md-12 col-12">
				<div class="accordion" id="accordionExample">
					...
					<?php get_template_part('template-parts/schema', 'faq'); ?>
				</div>
			</div>
		</div>
	</div>
		...
		<div class="row">
				...
				<div class="row">
					...
					<div class="col-lg-6 col-md-6 col-12">
						...
						<ul>
							...
							if ($selected_category): // Ensure a category is selected
							$args = array(
								'post_type'      => 'cities',  // Adjust if you're using a custom post type
								'posts_per_page' => -1,      // Limit to 10 posts
								'tax_query'      => array(
									array(
										'taxonomy' => 'cities_categories', // Change if using a custom taxonomy
										'field'    => 'term_id',
										'terms'    => $selected_category->term_id, // Get category ID from ACF
									),
								),
							...
							while ($query->have_posts()): $query->the_post(); 
							?>
							<li><a href="<?php the_permalink();?>" style="color:#fff !important; "><?php the_title();?></a></li>
							<?php 
							endwhile; 
							wp_reset_postdata(); 
							else:
			...
			<div class="col-lg-3 col-md-4 col-12">
				...
				<ul>
					<?php
					// Get the selected category from the ACF field on the current page
					$selected_category = get_sub_field('city_select_category'); 
